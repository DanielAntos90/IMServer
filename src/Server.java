...
class ActiveHandlers {
	...
	synchronized void sendPrivateMessage(SocketHandler sender, String message) {
		//"#sendPrivateMessage some message #sendTo ClientID"
		String[] data = message.split("#sendPrivateMessage")[1].trim().split("#sendTo");
		String text = data[0].trim();
		String to = data[1].trim();
		
		for (SocketHandler handler:activeHandlersSet)
			if (handler!=sender && handler.clientID.equals(to)) {
		...
		public void run() {
			try {
				...
				while ((request=reader.readLine())!=null) { 		// přišla od mého klienta nějaká zpráva?
						...
						if(request.contains("#setMyName")){
							clientID = request.split("#setMyName")[1].trim();
							System.out.println("New name set:"+clientID);
						} else if(request.contains("#sendPrivateMessage")) {
							activeHandlers.sendPrivateMessage(SocketHandler.this,request);
						} else {
